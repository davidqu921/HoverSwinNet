...
class TrainManager(Config):
    ...
    def view_dataset(self, mode="train"):
        ...
        target_info = phase_list["target_info"]    # ------------------调取Target_info - gen
        ...
        for batch_data in dataloader:  
            ...
            plt.imshow(viz)
            plt.savefig(f"/data4/userFolder/davidqu/study/hover_net-master/plot/view_train/training_process_{index}.png")
            index += 1
            print("Diagram Saved successf")
            plt.show()
        self.nr_gpus = -1
        return
    ...
    def _get_datagen(self, batch_size, run_mode, target_gen, nr_procs=0, fold_idx=0):
        ...
        input_dataset = FileLoader(     # <--------- 打包放入train_loader.py，使用obj[key] 或 obj[start:stop] 语法时，Python 会自动调用 __getitem__ 方法
            file_list,
            mode=run_mode,
        ...
        dataloader = DataLoader(   # <------------打包放入torch.utils.data的DataLoader
            input_dataset,
            num_workers=0,
    ...
    def run_once(self, opt, run_engine_opt, log_dir, prev_log_dir=None, fold_idx=0):
        ...
        ####
        loader_dict = {} 
        for runner_name, runner_opt in run_engine_opt.items():  # items() 通常用于遍历字典中的键值对
            loader_dict[runner_name] = self._get_datagen(    # <--------loader_dcit里面有run_engine的train和valid两个runner_name
                opt["batch_size"][runner_name],
                runner_name,
                opt["target_info"]["gen"],   # <--------- 进入opt.py，调取Target_info - gen，放入loader_dict
        ...
        runner_dict = {}
        for runner_name, runner_opt in run_engine_opt.items():  # <----------loader_dcit里面有run_engine的train和valid两个runner_name
        ...
        main_runner = runner_dict["train"]           # 设定模式为“train”，运行main_runners实际是在运行 train_step() in run_desc.py
        main_runner.state.logging = self.logging
        main_runner.state.log_dir = log_dir
        # start the run loop
        main_runner.run(opt["nr_epochs"])   # <--------------------------Run! 在这里进行每个文件的调用：obj[key]，从而得到完整的feed_dict
    ...
    def run(self):
        ...
        phase_list = self.model_config["phase_list"]   # /////////// 传入opt中"phase_list"参数 /////////////////
        engine_opt = self.model_config["run_engine"]   # /////////// 传入opt中"run_engine"参数 /////////////////

        prev_save_path = None
        for phase_idx, phase_info in enumerate(phase_list):   # ---------两次for循环代表"phase_list"中两个“run_info”，也就发生了两次run_once，对应生成两次checkpoint：00和01
            if len(phase_list) == 1:
