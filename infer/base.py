...
class InferManager(object):
    ...
    def __load_model(self):
        ...
        """
        # Import the model description module
        model_desc = importlib.import_module("models.hovernet.net_desc")
        model_creator = getattr(model_desc, "create_model")
        ...
        checkpoint = torch.load(self.method["model_path"], map_location=torch.device('cpu'), weights_only=True)
        print(f"Checkpoint content: {checkpoint}")  # Debugging statement

        # Ensure the checkpoint contains the expected state dictionary
        if "desc" not in checkpoint:
            raise ValueError(f"Checkpoint does not contain key 'desc'. Keys found: {checkpoint.keys()}")

        saved_state_dict = checkpoint["desc"]
        saved_state_dict = convert_pytorch_checkpoint(saved_state_dict)

        # Ensure the state dictionary is not None
        if saved_state_dict is None:
            raise ValueError("State dictionary is None. Check the checkpoint file and key used.")
        ...
        # Import the run description module
        module_lib = importlib.import_module("models.hovernet.run_desc")
        run_step = getattr(module_lib, "infer_step")
        self.run_step = lambda input_batch: run_step(input_batch, net)

        # Import the post-processing function
        module_lib = importlib.import_module("models.hovernet.post_proc")
        self.post_proc_func = getattr(module_lib, "process")
        return'
    '''
