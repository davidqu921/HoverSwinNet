...
def get_bounding_box(img):
    ...
    if np.any(rows) and np.any(cols):  # 检查是否存在非零元素
        rmin, rmax = np.where(rows)[0][[0, -1]]
        cmin, cmax = np.where(cols)[0][[0, -1]]
    else:
        # 如果 mask 是空的，返回默认值或抛出异常
        rmin, rmax, cmin, cmax = 0, img.shape[0], 0, img.shape[1]
        # 或者可以抛出一个异常
        # raise ValueError("The mask is empty and does not contain any non-zero elements.")
