...
class HoverSwinNetEnh(nn.Module):
    def __init__(
        self,
        img_size=256,
        in_channels=3,
        ...
        super(HoverSwinNetEnh, self).__init__()
        self.nr_types = (
        ...
        # 估计 decoder3 输出通道数，MONAI 默认 decoder3 = 384 通道（depends on feature_size）
        self.fused_channels = feature_size + 384

        # HoVerNet 分支输出，保留分支结构不变
        self.np_branch = HoVerNetBranch(in_channels=self.fused_channels, out_channels=out_channels_dict["np"], dropout_p=0.3)
        self.hv_branch = HoVerNetBranch(in_channels=self.fused_channels, out_channels=out_channels_dict["hv"], dropout_p=0.3)
        self.tp_branch = HoVerNetBranch(in_channels=self.fused_channels, out_channels=out_channels_dict["tp"], dropout_p=0.3)

    def forward(self, x):
        # 1. SwinUNETR 的输出
        feat, skip_feat = self.swin_unetr(x)  # now returns: (final_output, decoder3)  # Shape: B x C x H x W
        print("Shape of feat:", feat.shape)
        print("Shape of skip_feat (dec3):", skip_feat.shape)

        # Upsample skip_feat to match feat size
        skip_feat = F.interpolate(skip_feat, size=feat.shape[2:], mode="bilinear", align_corners=False)

        # Concatenate final output with mid-level decoder feature
        fused_feat = torch.cat([feat, skip_feat], dim=1)
        print("Shape of fused_feat:", fused_feat.shape)

        # 2. 三头分支
        np_out = self.np_branch(fused_feat)
        hv_out = self.hv_branch(fused_feat)
        tp_out = self.tp_branch(fused_feat)
