"""
================================================================================
Program Name: Nuclei Segmentation and Classification with MultiHead-HoVerNet
Version: 1.0
Author: David Qu
...
from .net_utils import (DenseBlock, Net, TFSamepaddingLayer, EnhancedResBlock, DenseEnhancement,
                        UpSample2x)
...
class EnhencedHoVerNet(Net):
    ...
    def __init__(self, input_ch=3, nr_types=None, freeze=False, mode='original'):  # <----- 将input channel改为5通道? No
        ...
        # 增强的残差阶段（保持原始下采样率）
        # 修改d1/d2阶段为混合结构
        self.d0 = self._make_enhanced_res_stage(64, 256, blocks=3)  # 纯CNN
        ...
        def create_decoder_branch(out_ch=2, ksize=5):
            module_list = [ 
                ...
                # ("dense_enhancement", DenseEnhancement(256, growth_rate=32, layers=4)),  # 增强特征
            ...
            module_list = [ 
                ...
                # ("dense_enhancement", DenseEnhancement(128, growth_rate=32, layers=4)),  # 增强特征
    ...
    def _make_enhanced_res_stage(self, in_ch, out_ch, blocks, use_se=False, stride=1):
        ...
        layers.append(nn.Sequential(
            EnhancedResBlock(in_ch, out_ch, stride=stride, use_se=use_se),
            # DenseEnhancement(out_ch, growth_rate=16)  # 轻量化设计
        ))
        # 后续块：常规增强块
