...
def train_step(batch_data, run_info):
    ...
    ####
    model = run_info["net"]["desc"]  # <--- create model 实例化模型
    ...
    # print("---image shape after permute:", imgs.shape)
    ...
    # print('---------------------------------------true_hv shape:', true_hv.shape)
    # print('---------------------------------------true_tp shape:', true_tp.shape)

    ####
    model.train()  #/////////////////// switch to training mode //////////////////////////
    model.zero_grad()  # not rnn so not accumulate

    pred_dict = model(imgs)     # ////////////////////////////////////////////////// Input image, get pred /////////////
...
def infer_step(batch_data, model):
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        print("Infer input size")
        print(patch_imgs_gpu.shape)
...
def viz_step_output(raw_data, nr_types=None):
    ...
    if len(true_np.shape) == 3 or len(pred_np.shape) == 3:
        true_np_expanded = np.expand_dims(true_np, axis=-1)
        pred_np_expanded = np.expand_dims(pred_np, axis=-1)
        aligned_shape = [list(imgs.shape), list(true_np_expanded.shape), list(pred_np_expanded.shape)]
    else:
        aligned_shape = [list(imgs.shape), list(true_np.shape), list(pred_np.shape)]
