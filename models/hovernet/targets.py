...
def gen_instance_hv_map(ann, crop_shape):
    ...
    # 使用 len() 函数查看列表的大小
    # inst_list_size = len(inst_list)
    # print("Size of inst_list:", inst_list_size)

    for inst_id in inst_list:
        inst_map = np.array(fixed_ann == inst_id, np.uint8)
        if np.sum(inst_map) == 0:  # 如果实例掩码为空，跳过
            # print(f"Skipping empty instance ID: {inst_id}")
            continue
...
####
def gen_targets(ann, crop_shape, **kwargs):  # <--------------从opt.py跳转过来, 为run_train提供gen_targets函数
...
def prep_sample(data, is_batch=False, **kwargs):
    ...
    def prep_one_sample(data):
        ...
        shape = np.maximum.reduce(shape_array)   # shape = np.maximum(*shape_array)
        viz_list = []
