...
class FileLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):  # 当使用 obj[key] 或 obj[start:stop] 语法时，Python 会自动调用 __getitem__ 方法。
        ...
        feed_dict = {"img": img} # <----------- 将RBG Images放入feed_dictionary

        inst_map = ann[..., 0]  # HW1 -> HW   <---------- 实例分割Lable,没有在这个阶段放入dict
        if self.with_type:
            type_map = (ann[..., 1]).copy()   # <---------- 类别分类Lable = type_map
            type_map = cropping_center(type_map, self.mask_shape)
            #type_map[type_map == 5] = 1  # merge neoplastic and non-neoplastic
            feed_dict["tp_map"] = type_map # <----------- 将type_map放入feed_dictionary

        # TODO: document hard coded assumption about #input
        target_dict = self.target_gen_func(    # <---------- 创建新的target_dict = 调用gen_targets（）在targets.py
            inst_map, self.mask_shape, **self.target_gen_kwargs
        )
        feed_dict.update(target_dict) # <----------将target_dict中的np_map和hv_map合并入feed_dict
